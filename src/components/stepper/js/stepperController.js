(function () {
	...
	angular.module('material.components.stepper').controller('MdStepperController', stepperController);

	/**
	 * @ngInject
	 */
	function stepperController($scope, $element, $window, $mdConstant, $mdStepInkRipple,
		...
		defineOneWayBinding('stretchStepper', handleStretchStepper);
		...
		defineBooleanAttribute('centerStepper', handleCenterStepper, true);
		...
		ctrl.hasFocus = false;
		ctrl.lastClick = true;
		ctrl.shouldCenterStepper = shouldCenterStepper();
		...
		function compileTemplate() {
			var template = $attrs.$mdStepperTemplate;
			var element = angular.element(elements.data);

			element.html(template);
			$compile(element.contents())(ctrl.parent);

			delete $attrs.$mdStepperTemplate;
		}
		...
		function configureWatchers() {
			$scope.$watch('$mdStepperCtrl.selectedIndex', handleSelectedIndexChange);
		}
		...
		/**
		 * Defines boolean attributes with default value set to true.  (ie. md-stretch-stepper with no value
		 ...
		 */
		function handleStretchStepper() {
			angular.element(elements.wrapper).toggleClass('md-stretch-stepper', shouldStretchStepper());
			updateInkBarStyles();
		}

		function handleCenterStepper() {
			ctrl.shouldCenterStepper = shouldCenterStepper();
		}
		...
		function handleOffsetChange(left) {
			var newValue = ctrl.shouldCenterStepper ? '' : '-' + left + 'px';
			angular.element(elements.paging).css($mdConstant.CSS.TRANSFORM, 'translate3d(' + newValue + ', 0, 0)');
			$scope.$broadcast('$mdStepperPaginationChanged');
		}
		...
		function handleSelectedIndexChange(newValue, oldValue) {
			...
			$scope.$broadcast('$mdStepperChanged');
		...
		function nextPage() {
			...
			var totalWidth = viewportWidth + ctrl.offsetLeft;
			var step;

			for (var i = 0; i < elements.steppers.length; i++) {
				step = elements.steppers[i];

				if (step.offsetLeft + step.offsetWidth > totalWidth) {
					break;
				}
			}
		...
		function previousPage() {
			...
			for (var i = 0; i < elements.steppers.length; i++) {
				...
				if (step.offsetLeft + step.offsetWidth >= ctrl.offsetLeft) {
					break;
				}
			}
		...
		function removeStep(stepData) {
			...
			var step = ctrl.steppers.splice(stepData.getIndex(), 1)[0];
		...
		function insertStep(stepData, index) {
			...
			var step = angular.extend(proto, stepData);
		...
		function getElements() {
			...
			elements.wrapper = $element[0].getElementsByTagName('md-stepper-wrapper')[0];
			elements.data = $element[0].getElementsByTagName('md-step-data')[0];
			elements.canvas = elements.wrapper.getElementsByTagName('md-stepper-canvas')[0];
			...
			elements.contentsWrapper = $element[0].getElementsByTagName('md-stepper-content-wrapper')[0];
		 ...
		 * @returns {*}
		 */
		function shouldStretchStepper() {
			switch (ctrl.stretchStepper) {
				case 'always':
					return true;
				case 'never':
					return false;
		...
		/**
		 * Determines if the stepper should appear centered.
		 * @returns {string|boolean}
		 */
		function shouldCenterStepper() {
			return ctrl.centerStepper && !ctrl.shouldPaginate;
		}
		...
		function getNearestSafeIndex(newIndex) {
			...
			for (var i = 0; i <= maxOffset; i++) {
				var step = ctrl.steppers[newIndex + i];
				if (step && (step.scope.disabled !== true)) {
		...
		function updateStepOrder() {
			...
			var focusItem = ctrl.steppers[ctrl.focusIndex];
		...
		function incrementIndex(inc, focus) {
			var newIndex;
			var key = (focus ? 'focusIndex' : 'selectedIndex');
			var index = ctrl[key];
		...
		function adjustOffset(index) {
			...
			var right = step.offsetWidth + left;
		...
		function updateHeightFromContent() {
			...
			if (!ctrl.steppers.length) {
				return queue.push(updateHeightFromContent);
			}

			var stepContent = elements.contents[ctrl.selectedIndex];
			var contentHeight = stepContent ? stepContent.offsetHeight : 0;
			var steppersHeight = elements.wrapper.offsetHeight;
			var newHeight = contentHeight + steppersHeight;
			var currentHeight = $element.prop('offsetHeight');
			...
			if ($element.attr('md-align-stepper') === 'bottom') {
		...
		function updateInkBarStyles() {
			...
			var totalWidth = elements.paging.offsetWidth;
			var step = elements.steppers[index];
			var left = step.offsetLeft;
			var right = totalWidth - left - step.offsetWidth;
			var stepWidth;

			if (ctrl.shouldCenterStepper) {
				stepWidth = Array.prototype.slice.call(elements.steppers).reduce(function(value, element) {
					return value + element.offsetWidth;
				}, 0);

				if (totalWidth > stepWidth) {
		...
		function updateInkBarClassName() {
			...
			var oldIndex = ctrl.lastSelectedIndex;
			var ink = angular.element(elements.inkBar);
		...
		function fixOffset(value) {
			if (!elements.steppers.length || !ctrl.shouldPaginate) {
				return 0;
			}

			var lastStep = elements.steppers[elements.steppers.length - 1];
			var totalWidth = lastStep.offsetLeft + lastStep.offsetWidth;
